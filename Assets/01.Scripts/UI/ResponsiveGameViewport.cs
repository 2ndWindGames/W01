...
namespace _01.Scripts.UI
{
    ...
    {
        private const float ReferenceAspect = 1080f / 2340f;
        ...
        private void Apply()
        {
            ...
            if (viewCamera.orthographic)
            {
                ...
                viewCamera.orthographicSize = originalSize * Mathf.Max(1f, ReferenceAspect / aspect);
            }
        }
        ...
        public static Rect CalculatePixelRect(float width, float height, Rect safe, float banner, float maxAspect)
        {
            ...
            // Rotation and Editor preview changes can leave a safe area from the old
            // resolution. Clamping its edges separately would shift the playfield.
            if (!(safe.width > 0f && safe.height > 0f && safe.xMin >= 0f && safe.yMin >= 0f
                && safe.xMax <= width && safe.yMax <= height))
            ...
            float boundedMaxAspect = Mathf.Max(0.1f, maxAspect);
            float playWidth = Mathf.Min(availableWidth, availableHeight * boundedMaxAspect);
            // Keep the HUD with the artwork in very tall split-screen windows. The
            // original playfield and target pixel sizes stay intact; surplus space
            // belongs outside the game rather than between its controls and targets.
            float playHeight = Mathf.Min(availableHeight, playWidth / Mathf.Min(ReferenceAspect, boundedMaxAspect));
            return new Rect(left + (availableWidth - playWidth) * 0.5f,
                bottom + (availableHeight - playHeight) * 0.5f, playWidth, playHeight);
        }
