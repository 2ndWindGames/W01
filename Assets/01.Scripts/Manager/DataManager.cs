...
namespace _01.Scripts.Manager
{
	...
	public class DataManager
	{
	  ...
	  //       Dialogues = dialogueLoader.MakeDic();
		...
		private TItem LoadSingleXml<TItem>(string name)
		{
			XmlSerializer xs = new XmlSerializer(typeof(TItem));
			TextAsset textAsset = Resources.Load<TextAsset>("Data/" + name);
			using (MemoryStream stream = new MemoryStream(System.Text.Encoding.UTF8.GetBytes(textAsset.text)))
				return (TItem)xs.Deserialize(stream);
		}

		private TLoader LoadXml<TLoader, TKey, TItem>(string name) where TLoader : ILoader<TKey, TItem>, new()
	    {
	        XmlSerializer xs = new XmlSerializer(typeof(TLoader));
	        TextAsset textAsset = Resources.Load<TextAsset>("Data/" + name);
	        using MemoryStream stream = new MemoryStream(System.Text.Encoding.UTF8.GetBytes(textAsset.text));
	        return (TLoader)xs.Deserialize(stream);
	    }
	}
}
