...
namespace _01.Scripts.Scene
{
	public class GameScene : BaseScene
	{
		...
		private void Update()
		{
			if (IsHelpOpen || mGameFlow.State != GameFlowState.Playing)
			{
				return;
			}
		...
		public void SetHelpOpen(bool open)
		{
			...
			foreach (CircleTarget target in m_ActiveTargets)
				if (target != null) target.SetPaused(open);
			m_TapFeedback?.SetPaused(open);
		}
		...
		private void EndFever(bool playSound = true)
		{
			...
			{
				m_StatusText.text = GameLocalization.T("TAP THE GLOWING TARGETS", "빛나는 타겟을 터치하세요");
